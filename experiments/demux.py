...
DATASET = {
    ...
    "MHS":DemuxDatasetForMHS,
    "MHSAnnotators":DemuxDatasetForAnnotators
}
TRAINER = {
    ...
    "MHS":DemuxTrainerForMHS,
    "MHSAnnotators":DemuxTrainerForAnnotators
}

MODEL = {
    "SemEval": Demux,
    "GoEmotions": Demux,
    "FrenchElections": Demux,
    "MHS":Demux,
    "MHSAnnotators":DemuxAnno
}
...
def parse_args():
    ...
    for task in DATASET:
        #print(task)
        #print(DATASET[task].argparse_args)
        sp_task = sp.add_parser(task)
        add_arguments(sp_task, MODEL[task].argparse_args)
...
def main():
    ...
    if task != "GoEmotions" or task != 'MHS':
    ...
    logging.info(f"Split?:  {args.split_all}")
    logging.info(f"Type split: {type(args.split_all)}")
    ...
    task_dataset_kwargs = (
        ...
        else dict(reddit_preprocessor=preprocessor)
    )
    ...
    for rep in range(reps):
        ...
        train_dataset = DATASET[task](
            ...
            seed=rep,
            platform=args.platform,
            split_all=args.split_all != 'False',
            **task_dataset_kwargs,
        )

        dev_dataset = DATASET[task](
                root_dir=args.root_dir,
                splits=args.dev_split,
                max_length=args.max_length,
                tokenizer=tokenizer,
        ...
        experiment_handler = ExperimentHandler(
            "./experiment_logs", f"Demux{task}_{platform}", description=description
        )
        ...
        model = MODEL[task].from_pretrained(
