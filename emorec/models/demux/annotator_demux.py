...
from transformers import AutoModel, PretrainedConfig, AutoConfig, AutoTokenizer


class DemuxAnno(nn.Module):
    """Demux.
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(config.hidden_size + 51, 128),
            nn.Tanh(),
    ...
    def forward(self, additional_input=None, class_inds=None, *args, **kwargs) -> torch.Tensor:
        """Forward propagation.
        ...
        #print("Args: ", args)
        #print("Kwargs: ", kwargs)
        #print("Class inds: ", additional_input)
        ...
        if not self.aggregate_logits or isinstance(class_inds, torch.Tensor):
            ...
            last_emotion_state = torch.stack(
                [
                    torch.cat((last_hidden_state.index_select(
                        dim=1,
                        index=(
                            torch.cat(inds) if isinstance(inds, list) else inds
                        ).to(last_hidden_state.device),
                    ).mean(1), additional_input), dim=1)
                    for inds in class_inds
                ],
                dim=1,
            )

            #print(last_emotion_state.shape)
