...
class EmotionDataset(Dataset, ABC):
    ...
    def __init__(
        ...
        texts, labels, platform_idx = self.load_dataset()
        self.labels = labels
...
class EmotionDatasetAdditionalFeatures(Dataset, ABC):
    """Base dataset class for multilabel emotion classification.
    ...
    def __init__(
        ...
        texts, labels, annotators, batches, platform_idx = self.load_dataset()
        self.labels = labels
        self.annotators = annotators
        self.eval_batches = batches
        self.inputs = (self.encode_plus(texts, **encode_kwargs), annotators)
        self.platform_idx = platform_idx
    ...
    @abstractmethod
    def load_dataset(self) -> Tuple[List[str], torch.Tensor, torch.Tensor]:
        """Loads text and labels (should stack if multiple splits are provided)."""
    ...
    def __getitem__(
        ...
        """Returns a single example in the `transformers` format
        and its labels."""
        return {k: v[index] for k, v in self.inputs[0].items()}, self.inputs[1][index], self.labels[index]

    def _gettext_(self, index: int) -> Tuple[str, Dict[str, float]]:
        """Returns a single example in text, and its labels in a dict."""
        inp, label, annotators = self.__getitem__(index)
    ...
    def collate_fn(self, batch) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]:
        """Returns multiple examples in the `transformers` format
        and their labels."""
        data, annotators, labels = [b[0] for b in batch], [b[1] for b in batch], [b[2] for b in batch]
        return (
            {k: torch.stack([d[k] for d in data]) for k in data[0]},
            torch.stack(annotators),
            torch.stack(labels)
        )
...
class MeasuringHateSpeechDataset(EmotionDataset, ABC):
    ...
    argparse_args.update(
        dict(
            ...
            platform=dict(
                type=str, help="Platform to use. Either 'Reddit' or 'all'", default="all"
            ),
            outputs=dict(
                type=str, help="Type of task to predict. Either 'hate' or 'identities'", default='hate'
            ),
            split_all=dict(
                type=str, help="Whether or not to train on the entire dataset (for inference) or to split.", default='True'
            )
        )
    )
    ...
    _emotions = [
                'negative', 'disrespectful', 'insult',
                'humiliation', 'superior', 'attack', 'dehumanizing',
    ...
    def load_dataset(self) -> Tuple[List[str], torch.Tensor]:
        ...
        df = pd.read_csv('./measuring_hate_speech_aggregated_labels.csv')
        ...
        y_train = df[self.emotions].values
        ...
        if self._split:
            ...
            texts = [x_train[i] for i in split_inds]
            labels = torch.from_numpy(y_train).float()[split_inds]
            platform_truth_index = platforms[split_inds] == 0

            return texts, labels, platform_truth_index

        else:
            return x_train, torch.from_numpy(y_train).float(), platforms == 0
    ...
    def encode_plus(
        ...
        return self.tokenizer.batch_encode_plus(
            [self.text_preprocessor(text) for text in texts],
            max_length=max_length,
            truncation="only_second",
            padding="max_length",
            return_tensors="pt",
...
class MHSAnnotators(EmotionDatasetAdditionalFeatures, ABC):
    """Measuring hate speech corpus.
    ...
    def __init__(
        ...
        """
        self.splits = splits
        self.tokenizer = self.update_tokenizer(tokenizer)
    ...
    def update_tokenizer(self, tokenizer):
        df = pd.read_csv('./mhs_unaggregated_annotators.csv')
    ...
    def load_dataset(self) -> Tuple[List[str], torch.Tensor, torch.Tensor, List[List[int]]]:
        """Loads text and labels from dataset `filenames`.
       ...
       'annotator_sexuality_straight', 'annotator_sexuality_other', 'annotator_age']
        ...
        ## Tasks: Load in dataset with matrix of annotator choices
        ## Add each annotator id (surrounded by brackets) to the tokenizer
        ...
        self.logger.info(f"the split: {self._split}")

        if self._split: #split the posts, not the individual annotations, to prevent data leakage
            _dev_size = max(1, int(self._dev_ratio * len(comment_ids)))
            _test_size = max(1, int(self._test_ratio * len(comment_ids)))
            #print(self.seed)
            random.seed(self.seed)
            eval_splits_inds = random.sample(
                range(len(comment_ids)), _dev_size + _test_size
            )
            train_split_inds = list(
                set(range(len(comment_ids))).difference(eval_splits_inds)
            )
            dev_split_inds = eval_splits_inds[:_dev_size]
            test_split_inds = eval_splits_inds[_dev_size:]
            ...
            kept_ids = [comment_ids[i] for i in split_inds]

        else:
            kept_ids = comment_ids

        mhs = df[df['comment_id'].isin(kept_ids)]
        mhs = mhs.sort_values(by='comment_id').reset_index(drop=True)

        mhs['text'] = '[' + mhs['annotator_id'].astype('str') + '] </s> ' + mhs['text'].astype('str') #concatenate a special token for each annotator

        x_train = mhs['text'].to_list()
        y_train = torch.from_numpy(mhs[self.emotions].values).float()

        annotator_data = torch.from_numpy(mhs[annotator_cols].values).float()

        grouped_mhs = mhs.groupby(['comment_id', 'platform']).apply(lambda x: x.index.tolist()).reset_index()

        platforms = grouped_mhs['platform'].values
        eval_batches = grouped_mhs[0].to_list()


        return x_train, y_train, annotator_data, eval_batches, platforms == 0
